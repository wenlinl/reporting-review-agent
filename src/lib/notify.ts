...
export async function generateNotifications(): Promise<number> {
  ...
  const done = await prisma.reminderRecord.findUnique({
    where: { type_scheduledFor: { type: "notify-daily", scheduledFor: today } },
  });
  if (done) return 0;
  ...
  if (expiryEnabled) {
    ...
    for (const it of [...targets, ...expired]) {
      const type = `expiry:${it.id}:${today.toISOString().slice(0, 10)}`;
  ...
  if (recipeEnabled) {
    ...
    if (recipe) {
      const type = `recipe:${today.toISOString().slice(0, 10)}`;
      const rec = await prisma.reminderRecord.findUnique({
        where: { type_scheduledFor: { type, scheduledFor: today } },
      });
      if (!rec) {
  ...
  await prisma.reminderRecord.create({
    data: { type: "notify-daily", scheduledFor: today },
  });
  return created;
}
