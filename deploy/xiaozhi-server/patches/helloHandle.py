...
async def checkWakeupWords(conn: "ConnectionHandler", text):
    ...
    # 修复：唤醒后立即开始 2 秒恢复计时，而不是等到下一帧音频到达才计时。
    # 否则“唤醒回复播完 → 设备静默等待用户开口”这段时间不会启动计时，
    # 用户下一句话会整段被 just_woken_up 吞掉（表现为“第二轮开始没反应”）。
    if not hasattr(conn, "vad_resume_task") or conn.vad_resume_task.done():

        async def _resume_vad_after_wakeup():
            await asyncio.sleep(2)
            conn.just_woken_up = False

        conn.vad_resume_task = asyncio.create_task(_resume_vad_after_wakeup())
